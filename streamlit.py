...
with st.sidebar:
    ...
    with st.form(ex):
        ...
        weight = st.number_input('Weight', value = np.nan, step = .5)
        distance = st.number_input('Distance', value = np.nan, step = .5)
        reps = st.number_input('Reps', value = 0, step = 1)
        RPE = st.number_input('RPE', value = np.nan, min_value = 0.0, max_value = 10.0, step = .5)
